...
$incremento = trim($_GET['cantidad']);
    ...
    if ($row_userid['ranking_coins'] < $gasto) {
            $respuesta->respuesta = "NOK";    
                ...
                $puntos = $valor * $incremento;

               $stmt3 = $con->prepare("UPDATE ranking SET ranking_".$tipo." = (ranking_".$tipo." + ".$incremento."), ranking_coins = (ranking_coins - ".$gasto.") WHERE ranking_users_id = (SELECT users_id FROM users WHERE users_name='".$username."')");

               $stmt3->execute();  
               $respuesta->respuesta = "OK";    

                 //SE AÑADE AL HISTORIAL
                $stmt10 = $con->prepare("INSERT INTO compraventa (compraventa_users_id_compra, compraventa_users_id_venta, compraventa_entrega_ud, compraventa_entrega_tipo, compraventa_recibe_ud, compraventa_recibe_tipo, compraventa_puntos) VALUES ((SELECT users_id FROM users WHERE users_name='".$username."'),0,".$gasto.",'coins',".$incremento.",'".$tipo."',".$puntos.")");
                $stmt10->execute(); 
